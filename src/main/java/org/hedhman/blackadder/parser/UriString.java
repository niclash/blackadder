    ...
    */
        ...
        throws URISyntaxException
    {
        ...
        for( char ch : chars )
        {
            ...
            else if( index( gen_delims, ch ) != -1
                     || index( sub_delims, ch ) != -1
                     || index( lowalpha, ch ) != -1
                     || index( upalpha, ch ) != -1
                     || index( numeric, ch ) != -1
                     || index( unres_punct, ch ) != -1 )
            {
                sb.append( ch );
            }
            else
            {
                int n = index( latin, ch );
                if( n < 0 )
                {
     ...
     */
    public static URI normalise( URI codebase )
        throws URISyntaxException
    {
        if( codebase == null )
        {
...
//        path = filePathNormalise(path);
     ...
     * @throws java.net.URISyntaxException
     */
    public static URI normalisation( URI uri )
        throws URISyntaxException
    {
        if( uri == null )
        {
