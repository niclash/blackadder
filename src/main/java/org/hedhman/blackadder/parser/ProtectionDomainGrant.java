...
class ProtectionDomainGrant extends PrincipalGrant
{
    ...
    public boolean equals( Object o )
    {
        ...
        if( o instanceof ProtectionDomainGrant )
        {
            ...
            if( domain != null && domain.equals( c.domain ) )
            {
                return true;
            }
        }
...
//        if (pd == null) return false;
    ...
    private boolean impliesProtectionDomain( ProtectionDomain pd )
    {
        ...
        return domain == null
               || pd != null
                  && domain.get() != null
                  && ( pd.equals( domain.get() )
                       || impliesClassLoader( pd.getClassLoader() )
    ...
    private boolean impliesClassLoader( ClassLoader cl )
    {
        if( domain == null )
        {
            return true;  // Dynamic grant compatibility
        }
        if( cl == null )
        {
            return false;
        }
        ProtectionDomain protectionDomain = domain.get();
        if( protectionDomain == null )
        {
            return false; // is void.
        }
        ClassLoader thisloader = protectionDomain.getClassLoader(); // pd not null
        return thisloader != null && thisloader.equals( cl );
    }
    ...
    private boolean impliesCodeSource( CodeSource codeSource )
    {
        ...
        if( pd == null )
        {
            return false; // is void - why did I have true?
        }
        ...
        if( cs == null )
        {
            return false; // Null cs indicates system domain, does not imply.
        }
        ...
        catch( URISyntaxException ex )
        {
            ...
            if( myLocation.toExternalForm().equals( hisLocation.toExternalForm() ) )
            {
                return true;
            }
        }
    ...
    {
        return super.isVoid() || domain != null && domain.get() == null;
    }
